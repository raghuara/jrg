import { Autocomplete, Box, Button, createTheme, Dialog, DialogActions, DialogContent, DialogTitle, Divider, Fab, Grid, Grid2, IconButton, InputAdornment, Paper, styled, Switch, Tab, Tabs, TextareaAutosize, TextField, ThemeProvider, Typography } from "@mui/material";
...
export default function CircularsApprovalPage() {
    ...
    const visibleCirculars = filteredScheduleData.filter(
    ...
    const handleSubmit = async (id) => {
        ...
        try {
            const res = await axios.put(
                `${updateCircularApprovalAction}?Id=${id}&RollNumber=${rollNumber}&UserType=${userType}&Action=accept&Reason=`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
    ...
    const handleDecline = async (id) => {
        ...
        try {
            const res = await axios.put(
                `${updateCircularApprovalAction}?Id=${id}&RollNumber=${rollNumber}&UserType=${userType}&Action=decline&Reason=${comments[id]}`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
