...
import { DashboardTeachersAttendance, fetchAllMarksStudents, fetchAttendance, MarksStudentsFetch, postAttendance, postMarks, sectionsDropdown, updateAttendance } from "../../../Api/Api";
...
export default function AddMarksPage() {
    ...
    useEffect(() => {
        if (selectedGradeId && isPosted !== "") {
            const status = isPosted === "Y" ? "post" : "draft";
            handleFetchDraft(status);
        }
    }, [selectedGradeId, isPosted, selectedSection, selectedExam]);
        ...
        const all_marksRequest = getDataStudents.map((row) => {
            ...
            return {
                ...
                ...getDataSubjects.reduce((subjects, subject) => {
                    ...
                    subjects[subjectKey] = studentMarks.subjects?.[subjectKey] || 0;
                    return subjects;
                }, {})
            };
        });
    ...
    const handleFetchDraft = async (status) => {
        ...
        try {
            ...
            const allSubjects = ["tamil", "english", "maths", "science", "social", "evs", "hindi", "phonics"];
            ...
            studentList.forEach((student) => {
                ...
                allSubjects.forEach((subj) => {
                    if (student.hasOwnProperty(subj)) {
                        subjects[subj] = Number(student[subj]);
                    }
                });

                const totalMarks = Object.values(subjects).reduce((sum, m) => sum + m, 0);
                const maxMarks = Object.keys(subjects).length * 100;
                const percentage = (totalMarks / maxMarks) * 100;
                const status = Object.values(subjects).every((m) => m >= 35) ? "Pass" : "Fail";
    ...
    return (
        <Box sx={{
            ...
            <Box>
                ...
                {/* <Box hidden={value !== 0}> */}
                <Box sx={{ px: 2, height: "100vh !important" }}>
                    <Box sx={{ display: "flex" }}>
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <TextField
                                                                size="small"
                                                                variant="standard"
                                                                value={marks[roll]?.subjects?.[subject.toLowerCase()] ?? ""}
                                                                onChange={(e) => handleMarksChange(roll, subject.toLowerCase(), e.target.value)}
                                                                InputProps={{
