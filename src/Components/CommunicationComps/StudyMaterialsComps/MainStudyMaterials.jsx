import { Autocomplete, Box, Button, DialogActions, Dialog, Fab, Grid, IconButton, Paper, Switch, TextField, Typography, ThemeProvider, createTheme, ToggleButtonGroup, ToggleButton, styled, Grid2, Tooltip } from "@mui/material";
...
export default function MainStudyMaterialsPage() {
    ...
    useEffect(() => {
        ...
        if (selectedGrade) {
            ...
            setSelectedSubject(null); // Clear selected subject when grade changes
        } else {
            setSubjectOptions([]);
        }
    }, [gradeId]);
    ...
    useEffect(() => {
        fetchTimeTables()
    }, [checked, selectedGradeId, selectedSection, formattedDate, selectedSubject])
                                                    ...
                                                    <Grid2 container sx={{ py: 1 }}>
                                                        <Grid2 sx={{ display: "flex", alignItems: "center" }} size={{ xs: 12, lg: 6 }}>
                                                            <Typography sx={{ fontSize: '14px', color: '#000', fontWeight: '600' }}>
                                                                {table.gradeSection} : {table.heading} - {table.subject}
                                                            </Typography>
                                                        </Grid2>
